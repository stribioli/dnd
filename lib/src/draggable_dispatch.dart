...
class _DragEventDispatcher {
  ...
  static EventStreamProvider<MouseEvent> enterEvent = new EventStreamProvider(CUSTOM_DRAG_ENTER);

  /// Stream provider for [CUSTOM_DRAG_OVER] events. The relatedTarget is empty.
  static EventStreamProvider<MouseEvent> overEvent = new EventStreamProvider(CUSTOM_DRAG_OVER);

  /// Stream provider for [CUSTOM_DRAG_LEAVE] events. The relatedTarget contains
  /// the [Element] the user is leaving to (may be null).
  static EventStreamProvider<MouseEvent> leaveEvent = new EventStreamProvider(CUSTOM_DRAG_LEAVE);

  /// Stream provider for [CUSTOM_DROP] events. The relatedTarget is empty.
  static EventStreamProvider<MouseEvent> dropEvent = new EventStreamProvider(CUSTOM_DROP);
  ...
  static void dispatchEnterOverLeave(Draggable draggable, EventTarget target) {
    ...
    if (previousTarget == target) {
      ...
      MouseEvent dragEnterEvent = new MouseEvent(CUSTOM_DRAG_ENTER, relatedTarget: previousTarget);
      target.dispatchEvent(dragEnterEvent);

      // Fire dragLeave of old element (if there is one).
      if (previousTarget != null) {
        MouseEvent dragLeaveEvent = new MouseEvent(CUSTOM_DRAG_LEAVE, relatedTarget: target);
