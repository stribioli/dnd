...
abstract class AvatarHandler {
  ...
  void cacheMargins() {
    ...
    _marginLeft = num.parse(computedStyles.marginLeft.replaceFirst('px', ''), (s) => 0);
    _marginTop = num.parse(computedStyles.marginTop.replaceFirst('px', ''), (s) => 0);
  }
}
...
class OriginalAvatarHandler extends AvatarHandler {
  ...
  void dragEnd(Point startPosition, Point position) {
    ...
    Point constrainedPosition = new Point(math.max(1, position.x), math.max(1, position.y));
